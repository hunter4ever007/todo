...
var worker_default = {
  async fetch(request, env2) {
    ...
    try {
      if (path === "/api/init") return cors(handleInit(env2));
      if (path === "/api/subscribe" && request.method === "POST") return cors(handleSubscribe(request, env2));
      if (path === "/api/schedule" && request.method === "POST") return cors(handleSchedule(request, env2));
      if (path === "/api/cancel" && request.method === "POST") return cors(handleCancel(request, env2));
      if (path === "/api/due") return cors(handleDue(request, env2));
  ...
  async scheduled(event, env2) {
    await processDueTasks(env2);
  }
};
async function handleInit(env2) {
  let keys = await env2.KV_SCHEDULES.get("vapid-keys", "json");
  if (!keys) {
    ...
    await env2.KV_SCHEDULES.put("vapid-keys", JSON.stringify(keys));
  }
...
async function handleSubscribe(request, env2) {
  ...
  await env2.KV_SCHEDULES.put(key, JSON.stringify(sub));
  await env2.KV_SCHEDULES.put(`sub-rev:${b64urlEncode(new TextEncoder().encode(sub.endpoint))}`, String(Date.now()));
  return json({ ok: true });
}
__name(handleSubscribe, "handleSubscribe");
async function handleSchedule(request, env2) {
  ...
  await env2.KV_SCHEDULES.put(`sch:${task.id}`, JSON.stringify({ task, endpoint, minute }));
  const raw = await env2.KV_SCHEDULES.get(`due:${minute}`);
  const ids = raw ? JSON.parse(raw) : [];
  if (!ids.includes(task.id)) {
    ids.push(task.id);
    await env2.KV_SCHEDULES.put(`due:${minute}`, JSON.stringify(ids));
  }
  return json({ ok: true });
}
__name(handleSchedule, "handleSchedule");
async function handleCancel(request, env2) {
  const { id } = await request.json();
  if (!id) return json({ error: "missing id" }, 400);
  const sch = await env2.KV_SCHEDULES.get(`sch:${id}`);
  if (sch) {
    ...
    if (minute) {
      ...
      if (raw) {
        ...
        if (ids.length > 0) await env2.KV_SCHEDULES.put(`due:${minute}`, JSON.stringify(ids));
        else await env2.KV_SCHEDULES.delete(`due:${minute}`);
      }
    }
    await env2.KV_SCHEDULES.delete(`sch:${id}`);
  }
  return json({ ok: true });
}
__name(handleCancel, "handleCancel");
async function handleDue(request, env2) {
  const endpoint = request.headers.get("x-push-endpoint");
  if (!endpoint) return json({ tasks: [] });
  const tasks = [];
  let cursor;
  do {
    const list = await env2.KV_SCHEDULES.list({ prefix: "sch:", cursor });
    cursor = list.cursor;
    for (const key of list.keys) {
      const val = await env2.KV_SCHEDULES.get(key.name);
      if (!val) continue;
      const { task, endpoint: ep } = JSON.parse(val);
      if (ep !== endpoint) continue;
      const target = (/* @__PURE__ */ new Date(task.date + "T" + task.time)).getTime();
      if (Date.now() >= target - 6e4) {
        tasks.push(task);
        await env2.KV_SCHEDULES.delete(`sch:${task.id}`);
        await removeFromDueList(env2, task.id, task.date, task.time);
      }
    }
  } while (cursor);
  return json({ tasks });
}
__name(handleDue, "handleDue");
async function removeFromDueList(env2, id, date, time3) {
  const minute = makeMinuteKey(date, time3);
  if (!minute) return;
  const raw = await env2.KV_SCHEDULES.get(`due:${minute}`);
  if (!raw) return;
  const ids = JSON.parse(raw).filter((x) => x !== id);
...
__name(removeFromDueList, "removeFromDueList");
async function processDueTasks(env2) {
  const minute = getCurrentMinuteKey();
  const raw = await env2.KV_SCHEDULES.get(`due:${minute}`);
  if (!raw) return;
  const ids = JSON.parse(raw);
  const vapidKeys = await env2.KV_SCHEDULES.get("vapid-keys", "json");
  if (!vapidKeys) return;
  for (const id of ids) {
    const sch = await env2.KV_SCHEDULES.get(`sch:${id}`);
    if (!sch) continue;
    ...
    const subRaw = await env2.KV_SCHEDULES.get(key);
    if (!subRaw) continue;
    ...
    try {
      ...
      if (e.message.includes("410") || e.message.includes("404")) {
        await env2.KV_SCHEDULES.delete(key);
        await env2.KV_SCHEDULES.delete(`sub-rev:${b64urlEncode(new TextEncoder().encode(endpoint))}`);
      }
      continue;
    }
    await env2.KV_SCHEDULES.delete(`sch:${id}`);
  }
  await env2.KV_SCHEDULES.delete(`due:${minute}`);
}
__name(processDueTasks, "processDueTasks");
...
function makeMinuteKey(date, time3) {
  if (!date || !time3) return null;
  const d = /* @__PURE__ */ new Date(date + "T" + time3);
  if (isNaN(d.getTime())) return null;
  return fmtMinute(d);
}
__name(makeMinuteKey, "makeMinuteKey");
function getCurrentMinuteKey() {
  return fmtMinute(/* @__PURE__ */ new Date());
}
__name(getCurrentMinuteKey, "getCurrentMinuteKey");
function fmtMinute(d) {
  const p = /* @__PURE__ */ __name((n) => String(n).padStart(2, "0"), "p");
  return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}-${p(d.getHours())}:${p(d.getMinutes())}`;
}
__name(fmtMinute, "fmtMinute");
...
__name(jwkToRawPublicKeyBytes, "jwkToRawPublicKeyBytes");
